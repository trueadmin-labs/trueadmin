...
const flattenDepartments = (departments: DepartmentTreeNode[]): DepartmentTreeNode[] =>
  departments.flatMap((department) => [department, ...flattenDepartments(department.children ?? [])]);

const toPageResult = (items: DepartmentTreeNode[], params: CrudListParams): PageResult<DepartmentTreeNode> => ({
  items,
...
export const departmentApi = {
  ...
  tree: (params?: CrudListParams) => http.Get<DepartmentTreeNode[]>('/admin/system/departments/tree', params ? { params } : undefined).send(),
  detail: (id: React.Key) => http.Get<DepartmentTreeNode>(`/admin/system/departments/${id}`).send(),
  create: (payload: DepartmentPayload) => http.Post<DepartmentTreeNode>('/admin/system/departments', payload).send(),
  update: (id: React.Key, payload: DepartmentPayload) => http.Put<DepartmentTreeNode>(`/admin/system/departments/${id}`, payload).send(),
