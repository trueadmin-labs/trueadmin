...
原因是官方完整模板和 simple 精简模板仍包含大量演示资产，例如 Welcome/Admin/table-list/chatbot/PWA/service-worker、多语言示例、Pro 官网链接和示例 Mock。TrueAdmin Web 要保持框架产品的最小可维护面，所以保留当前 `foundation + modules` 结构，只吸收官方模板中必要的依赖、构建配置和运行时实践。

## 目录规范

`web` 是纯后台管理端，不在内部再划分 `admin` / `client`。用户端前端如果未来需要独立建设，应新建独立应用目录。

当前采用 `foundation + modules` 组织：
