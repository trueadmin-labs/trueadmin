...
接口元数据体系见 `docs/backend/interface-metadata.md`。当前已提供并启用 `AdminController`、`AdminGet/AdminPost/AdminPut/AdminDelete`、`Client*`、`Open*`、`Permission`、`MenuButton`、`OpenApi` 等 Attribute。路由注解已参与运行时注册，Controller 注解 `path` 即完整控制器资源路径；菜单、权限和 OpenAPI 元数据已可通过 `trueadmin:metadata-sync` 同步或导出。
