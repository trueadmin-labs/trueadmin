...
后端主入口见 `docs/backend/index.md`。层级边界见 `docs/backend/layer-boundaries.md`，数据库迁移规范见 `docs/backend/database-migrations.md`，初始化主入口使用 `trueadmin:init`，迁移仍使用 Hyperf 原生 `migrate`，模块/插件路径由启动监听器注册，模块 Seeder 由初始化命令扫描执行。后端完整架构见 `docs/backend/architecture.md`，注解驱动设计见 `docs/backend/annotation-driven.md`。API 版本和多端复用边界见 `docs/backend/module-architecture.md` 与 `docs/api/versioning-and-reuse.md`。当前阶段使用 `kernel + Foundation + Infrastructure + Module` 作为后端主结构，并在业务模块内部采用 MineAdmin 分层。
...
接口元数据体系见 `docs/backend/interface-metadata.md`。当前已提供并启用 `AdminController`、`AdminGet/AdminPost/AdminPut/AdminDelete`、`Client*`、`Open*`、`Permission`、`MenuButton`、`OpenApi` 等 Attribute，其中路由注解已参与运行时注册；菜单和权限同步到数据库仍以后续 MetadataScanner 落库能力为准。
