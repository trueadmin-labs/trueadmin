...
数据库迁移只扫描模块和插件目录。第一版不保留 `backend/database/migrations` 根级迁移目录；系统基础表归属 `Module/System`，用户端身份表归属 `Module/User`，业务表归属对应业务模块。

后端主入口见 `docs/backend/index.md`。层级边界见 `docs/backend/layer-boundaries.md`，数据库迁移规范见 `docs/backend/database-migrations.md`，主入口使用 Hyperf 原生 `migrate`，模块/插件路径由启动监听器注册。后端完整架构见 `docs/backend/architecture.md`，注解驱动设计见 `docs/backend/annotation-driven.md`。API 版本和多端复用边界见 `docs/backend/module-architecture.md` 与 `docs/api/versioning-and-reuse.md`。当前阶段使用 `kernel + Foundation + Infrastructure + Module` 作为后端主结构，并在业务模块内部采用 MineAdmin 分层。
...
配置模式与注解模式边界见 `docs/backend/configuration-vs-attribute.md`。当前原则是：框架本身优先配置模式，业务开发优先注解模式，模块资产优先模块清单。路由需要同时支持配置模式和注解模式，第一版优先实现模块 `routes.php` 扫描以解决 `config/routes.php` 过于集中的问题，后续再叠加注解路由。

模块配置路由已落地：`backend/config/routes.php` 只保留框架入口、OpenAPI 入口和 `ModuleRouteRegistrar`，业务路由拆到各模块 `routes.php`。可用 `php bin/hyperf.php trueadmin:route-files` 查看扫描结果。

接口元数据体系见 `docs/backend/interface-metadata.md`。当前已提供 `AdminController`、`AdminGet/AdminPost/AdminPut/AdminDelete`、`Client*`、`Open*`、`Permission`、`MenuButton`、`OpenApi` 等 Attribute 作为设计契约，但暂不自动注册注解路由，也不自动同步菜单和权限到数据库。下一步应先实现 MetadataScanner，扫描输出清单，再逐步接入注解路由和模块资产同步。
