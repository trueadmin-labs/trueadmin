...
角色是平铺的权限集合，不表达组织层级、岗位上下级或权限继承关系：
...
菜单、按钮、接口权限和数据权限都必须显式授权到角色。用户拥有多个角色时，运行时合并这些角色的权限。普通管理员后续如果需要限制可授权范围，应基于当前 operator 的有效权限边界或专门的授权策略校验，不通过角色层级表达。`super-admin` 是内置特殊角色，可以绕过功能权限校验，但仍应保留操作日志。
