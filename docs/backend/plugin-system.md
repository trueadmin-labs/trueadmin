...
- 根 `plugin.json` 不承载 npm、Composer 或其他 runtime 依赖；框架级插件 CLI 读取各端依赖文件生成安装计划。
...
`backend/php/composer.json` 只描述 PHP runtime 自己的依赖和 autoload，不是 TrueAdmin 插件包清单。插件后端目录必须和 `backend/app/Module/*` 模块目录保持同构，业务类目录直接放在 `backend/php` 根下，不再额外包一层 `src/`。`web/package.json` 只描述 Web runtime 自己额外需要的 npm 依赖。Web runtime 不允许再放自己的 `plugin.json`；`web/manifest.ts` 只负责前端运行时注册。框架级插件 CLI 复制 runtime 到各端运行时目录，Windows 下不使用软链。
...
- `enabled` 是安装时默认启用建议；框架级插件 CLI 会写入根目录 `plugins.config.json`，并分发生成各端自己的运行时配置文件。
...
框架级插件 CLI 负责读取这些 runtime 依赖文件，合并到宿主项目安装计划。根 `plugin.json` 只校验插件包级依赖。
...
- 后端只读取自己端内的 `backend/config/autoload/plugins.php`，不读取根目录 `plugins.config.json`，也不读取 Web 或 Mobile 配置。
...
```json
{
  "installed": {
    "true-admin.examples": {
      "source": "plugins/true-admin/examples",
      "backendPath": "backend/plugins/true-admin/examples",
      "webPath": "web/src/plugins/true-admin/examples",
      "version": "0.1.0",
      "enabled": true,
      "defaults": {}
    }
...
后端运行时配置由 `trueadmin plugin sync` 生成到 `backend/config/autoload/plugins.php`：
...
```text
plugins.config.json installed.<plugin>.defaults  插件安装后的默认配置。
plugins.config.json config.<plugin>              项目覆盖配置，归宿主项目维护。
```
...
- 框架级插件 CLI 可以从插件包中的模板或约定文件读取默认配置，再写入 `plugins.config.json` 的 `installed.<plugin>.defaults`。
- 项目差异写在 `plugins.config.json` 的 `config`，再由 CLI 分发到各端自己的配置文件。
- 不把项目私有配置写回插件包源目录。
...
TrueAdmin 插件 CLI 属于框架级工具，不属于 backend、web 或 mobile 任一端。它负责读取根 `plugin.json` 校验包级信息，并读取各端依赖文件生成依赖安装计划。

插件 CLI 同时负责复制 runtime 资产：
...
第一版建议只实现开发者/部署期命令，不实现后台 UI 动态维护。正式入口是 `trueadmin`，仓库内的 npm scripts 只是开发期包装：

```bash
trueadmin plugin list
trueadmin plugin validate
trueadmin plugin install vendor/name
...
Web 插件资产源放在插件包的 `web/` 目录。框架级插件 CLI 复制 `plugins/<vendor>/<name>/web` 到 `web/src/plugins/<vendor>/<name>`，当前第一阶段由 Vite 只扫描安装后的目录：
...
- 不允许框架级插件 CLI 之外的代码直接修改 `backend/app`、`web/src` 或 `mobile` 源码。
