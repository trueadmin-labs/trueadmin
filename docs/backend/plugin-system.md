...
`backend/php/composer.json` 只描述 PHP runtime 自己的依赖和 autoload，不是 TrueAdmin 插件包清单。`web/package.json` 只描述 Web runtime 自己额外需要的 npm 依赖。Web runtime 不允许再放自己的 `plugin.json`；`web/manifest.ts` 只负责前端运行时注册。安装器复制 runtime 到各端运行时目录，Windows 下不使用软链。
