...
角色使用平铺模型，不使用父子级树结构。角色只表示一组显式权限集合：菜单/按钮权限、接口权限和数据权限策略。组织层级由部门树表达，导航层级由菜单树表达，角色不再承担组织关系、岗位上下级或授权继承语义。
...
- 角色之间不存在父子级约束。
- 菜单、按钮和接口权限必须显式授权到角色。
- 数据权限必须按资源、动作、策略显式授权到角色。
- 用户拥有多个角色时，运行时合并多个角色的功能权限和数据权限。
- `super-admin` 是内置特殊角色，可以绕过功能权限校验，但仍记录操作日志。
- 普通管理员的可授权范围不靠角色树表达，而应由当前 operator 的有效权限边界、管理能力或后续专门的授权策略校验承载。

为什么不用角色树：后台角色经常用于岗位职责拆分，例如财务、业务员、仓管、跟单员。它们之间不是稳定的上下级关系，数据权限也不天然包含。使用角色树容易把菜单权限、组织关系、数据范围和管理边界混在一起，导致授权语义变得含糊。平铺角色配合显式数据策略，更适合 ERP 这类多模块、多岗位、多数据域的系统。
...
数据权限不提供隐式默认值，也不使用 `*/*` 通配兜底。框架提供策略能力，模块负责显式注册资源；只有注册过的资源才能在角色授权中配置数据权限。某个 Repository 调用了 `applyDataPolicy()` 或 `assertDataPolicyAllows()`，但资源未注册时，按配置错误直接抛异常，避免开发阶段静默放行或误拒绝。
...
scope       all / self / department / department_and_children / custom_departments / custom_departments_and_children
config      自定义部门类 scope 使用 { deptIds: [...] }
effect      预留 allow / deny，第一版以 allow 并集为主
```

`custom_departments` 表示指定部门直属数据，只按保存的 `deptIds` 过滤；`custom_departments_and_children` 表示指定部门及子部门数据，保存的仍然只是用户明确选择的 `deptIds`，查询时由后端动态展开子部门。这样新增子部门后不需要重新配置角色。

角色可以拥有多条数据策略。多角色、多策略按 allow 并集组合；某个资源的策略为 `all` 时不追加查询约束。后续启用 deny 时，deny 策略应优先排除。超级管理员拥有所有已注册数据资源的 `all` 权限，但实现上按注册表展开为具体规则，不引入全局通配。

模块或插件通过后端运行时的 `resources/data_policies.php` 注册数据权限能力。文件格式和菜单资源保持一致，由框架自动扫描 `backend/app/Module/*/resources/data_policies.php` 和已启用插件的 `resources/data_policies.php`，业务模块不需要修改宿主 `config/autoload/data_policy.php`：
...
return [
    'strategies' => [
        App\Module\Erp\Service\DataPermission\CustomerDataPolicyStrategy::class,
    ],
    'resources' => [
        [
            'key' => 'erp_sales_order',
...
`config/autoload/data_policy.php` 只作为项目级全局追加入口使用，框架内置模块和第三方插件都应优先随模块提供 `resources/data_policies.php`。注册表会严格校验重复 resource、未注册 strategy、空 key 和旧式 action 维度，确保数据权限保持资源级模型。

策略注册元数据必须包含稳定 key、fallback label 和 i18n key。前端角色授权页通过 `/api/admin/system-config/data-policies/metadata` 获取资源、策略和 scope 元数据，再用当前语言包渲染。审计日志不按这套元数据做多语言，日志记录保持当时发生的事实文本和结构。

策略可以实现 `contains(parent, child)` 这类包含关系能力，用于未来显式的委派授权、审批边界或策略比较场景。默认角色授权流程不调用它，也不根据角色关系限制数据权限；数据权限始终是角色自身的显式配置，用户拥有多个角色时按策略合并。

`#[DataScope]` 的职责不是写死“本部门/本人”，而是声明当前方法涉及的数据资源。列表、详情、编辑、删除这类能力由 RBAC 的菜单/按钮权限控制；数据权限只表达该资源可见和可操作的数据范围：
...
```php
$this->applyDataPolicy($query, 'outbound_order', [
    'deptColumn' => 'dept_id',
...
详情、编辑、删除和批量操作必须使用同一资源策略做断言：

```php
$this->assertDataPolicyAllows($query, 'outbound_order', [
    'deptColumn' => 'dept_id',
    'createdByColumn' => 'created_by',
]);

$this->assertDataPolicyAllowsAll($query, 'outbound_order', $ids, 'id', [
    'deptColumn' => 'dept_id',
    'createdByColumn' => 'created_by',
]);
```
...
模块注册资源后，角色授权页会自动出现这些资源和策略。资源不再声明 list/detail/update/delete 这类 action，功能动作由 RBAC 按菜单、按钮和接口权限控制；数据权限只配置该资源的数据范围：
...
admin_roles        不走部门数据权限，使用角色自身管理权限和授权边界
