...
本文档定义 TrueAdmin 后端在 `trueadmin-kernel`、`backend/app/Foundation`、`backend/app/Infrastructure` 和 `backend/app/Module` 之间的职责边界。
...
它承接少量项目默认行为，不作为 Composer 包发布，开发者可以直接修改。已经稳定进入 `trueadmin-kernel` 的能力不再放回 Foundation。
...
```text
框架级生命周期监听器进入 trueadmin-kernel。
少量项目级基础命令或监听器只有在确实不适合进入 kernel、Infrastructure 或 Module 时，才进入 backend/app/Foundation 的具体能力目录；当前模板不保留这类命令或监听器。
...
```text
Foundation   项目级默认行为，当前只保留健康检查和 Repository 组合层
...
- 如果是跨项目稳定原语，放 `trueadmin-kernel`。
- 如果是项目可改默认行为，且不是已稳定进入 kernel 的通用能力，才放 `backend/app/Foundation`。
