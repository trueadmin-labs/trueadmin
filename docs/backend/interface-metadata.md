...
模块清单负责安装、升级、生成器和 AI 理解。
数据库负责运行时真实状态。
```

第一版先提供 kernel 级 Attribute 设计草案，不直接启用注解路由注册。当前运行路由仍以模块 `routes.php` 配置模式为准。
...
当前这些路由注解只作为元数据声明。后续注解路由注册器启用后，优先级为：

```text
模块 routes.php 显式配置 > 注解路由
```
...
#[OpenApi(
    ...
    tags: ['商品管理'],
    security: ['admin'],
    errorCodes: ['UNAUTHORIZED', 'VALIDATION_FAILED'],
)]
```
