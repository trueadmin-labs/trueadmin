...
- 对 kernel `CrudQueryRequest` 的项目级 Request 子类
- `AbstractRepository`
- 项目默认端点，例如 `HealthController`
- 项目默认 Repository 组合层，以及 `applyParams()`、`filterColumn()`、`sortColumn()` 等模板覆盖点
- 尚未稳定进入 kernel 的框架默认实现

不允许：

- 复制已经稳定进入 `trueadmin/kernel` 的值对象
- 复制已经稳定进入 `trueadmin/kernel` 的分页、API envelope、FormRequest、CrudQueryRequest、DataPolicy runtime、Controller 默认基类、Permission middleware runtime、Service helper base、默认 ExceptionHandler 和运行时服务
- 承载模块业务事实
