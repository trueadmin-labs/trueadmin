...
- HTTP Controller 默认基类，例如 `AbstractController`、`AdminController`、`ClientController`、`OpenController`
- HTTP 权限中间件和权限校验契约，例如 `PermissionMiddleware`、`PermissionProviderInterface`
- 默认 API 异常处理器 `TrueAdminExceptionHandler`
- Actor / Context 值对象和 `ActorFactory`
- 数据库 Model 基类、事务提交后回调 helper 和 listener
- 数据权限和操作日志的 Attribute、事件和值对象
- 数据权限 runtime，例如 `DataPolicyManager`、`DataPolicyRegistry`、`DataPolicyProviderInterface`、`DataPolicyStrategyInterface`
- CRUD 查询协议值对象，例如 `CrudQuery`、`CrudFilterCondition`、`CrudSortRule`
- CRUD 查询应用器，例如 `CrudQueryApplier`、`CrudQueryApplierOptions`
- Service helper base，例如 `AbstractService` 的通用校验和 not found helper
- 支持类，例如 `Password`、`TreeHelper`
- kernel 通用语言资源，例如 `KERNEL.*` 错误文案
...
- 业务 Controller、Request、Service、Repository
...
模板项目可改的后端默认实现层。它可以依赖 `trueadmin/kernel`，但只保留当前模板仍需要直接拥有的默认行为。

允许：

- `Repository/AbstractRepository.php`：项目默认 Repository 组合层，以及 `applyParams()`、`filterColumn()`、`sortColumn()` 等模板覆盖点
- `Http/Controller/HealthController.php`：模板默认健康检查端点
- `DataPermission/Organization/*`：模板默认组织数据权限策略和数据来源接口。策略只依赖 kernel 数据权限契约；具体部门树数据由 System 或项目通过接口提供
- 项目确实需要覆盖 kernel 行为时新增的项目类，例如通过 DI/config 替换的异常处理、OpenAPI builder 或 Controller 基类；这类类必须是项目覆盖实现，不是 kernel 源码副本

不允许：

- 复制已经稳定进入 `trueadmin/kernel` 的值对象
- 复制已经稳定进入 `trueadmin/kernel` 的分页、API envelope、FormRequest、CrudQueryRequest、DataPolicy runtime、Controller 默认基类、Permission middleware runtime、Service helper base、默认 ExceptionHandler、ActorFactory、Model 基类、Password、TreeHelper、AfterCommit callbacks、kernel 语言包和运行时服务
- 承载模块业务事实
